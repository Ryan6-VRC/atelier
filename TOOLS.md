...
| `mark_coverage` | Mark the body triangles an explicit garment set hides by proximity, skinning to the body's own bones, and a cone of peek rays that must not escape (no camera, no pose sample) and emit the Modular Avatar Delete carrier shape key for them; `--whatif` reports and renders the marked body, the real run writes the key into the base blend the linked body resolves to, refusing a changed body. Behavior: `blender.md`. |
