...
| `ReportConsole` | The Editor console, every line of every entry, with byte-identical entries collapsed by default; the MCP `read_console` tool is denied in its favour. Contract: `unity-tools.md`. |
...
| `ReportAvatarRecord` | An uploaded avatar's **live** blueprint record as the server holds it — published name, description, release status, bundle and thumbnail presence. The scene is not evidence about what is published; this is. Ids and URLs never enter output. Async: `Run`, then poll `Status()`. Contract: `unity-tools.md` §Publish. |
| `RenameAvatarRecord` | Rename an already-uploaded avatar — **metadata only, no bundle, no re-upload**, which is the one thing a re-upload structurally cannot do. Requires `expectCurrentName` and refuses on mismatch, so a stale `blueprintId` cannot land the rename on a different published avatar; chain it from `ReportAvatarRecord`'s reported name, which is the server-sanitized form. `whatIf` previews. Contract: `unity-tools.md` §Publish. |
