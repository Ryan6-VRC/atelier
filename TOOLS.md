...
| `UpdateAvatarRecord` | Edit an already-uploaded avatar's name, description, tags and **thumbnail** — no bundle, no re-upload. Fields are null-means-unchanged (a thumbnail can be replaced but never cleared); `expectCurrentName` is required and chains from `ReportAvatarRecord`. An image makes the call **two writes that can half-succeed**, so it reports per field. `ReleaseStatus` is deliberately not settable. `whatIf` previews. Contract: `unity-tools.md` §Publish. |
