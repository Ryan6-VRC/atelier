...
| `mark_coverage` | Mark the body triangles an explicit garment set hides — rays from the skin in a cone, blocked only by garment faces on the skin's own bones (kin steps and a fold glob widen "own"), the configuration's shapes applied first — and emit the Modular Avatar Delete carrier shape key at polygon granularity; `--whatif` reports, renders and can save an inspect blend, the real run writes the key into the base blend the linked body resolves to, refusing a changed body. Behavior: `blender.md`. |
