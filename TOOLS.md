...
| `CheckSeam` | Gates world-position coincidence of weighted humanoid bones within tolerance, naming worst-first offenders. `Check` takes a base + placed mergeable and reflects the MA/VRCFury seam mapping; `CheckBare` takes two side-by-side skeletons with no seam yet (a fresh refit output, an unplaced mergeable), pairs by bone name, and requires an explicit `maxOffsetMm`. Certifies the humanoid skeleton coincides, not accessory placement; inspection-only, the mechanical fit gate to run before any render. |
