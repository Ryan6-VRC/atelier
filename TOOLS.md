...
| `ReportDeleteFate` | Whether each declared geometry removal on a composed avatar actually removed anything: per MA `ShapeChanger` Delete row and `MeshCutter` a fate measured on a fresh build — `build-time` / `NaNimation` / `dropped` / `cancelled-by-Set` — with the controlling parameters and the mechanism behind every non-`build-time` one, plus the built triangle count against a budget. **NaNimation keeps every vertex and every triangle**, so this is the door that says whether an anti-clip Delete bought anything; `ReportShapeOverlap` reads the authoring half and by contract cannot answer this. Bakes, so **two-phase** (`ReportDeleteFate.Run` → `ReportDeleteFate.Verify`). PASS/CLASSIFY. Contract: `unity-tools.md`. |
