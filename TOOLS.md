...
| `RenderThumbnail` | Baked posed portrait for an avatar's upload thumbnail — 1200x900 PNG, **edit-mode** default: bakes the **full VRC SDK preprocess chain** (optimizers included), so it shows what actually uploads; `RenderAvatar` never bakes. One deterministic synchronous call. `pose` and optional `expression` are names, and an unknown one enumerates. `png=` feeds `UpdateAvatarRecord` — `UploadAvatar` takes no image. |
...
| `mark_coverage` | Mark the body triangles an explicit garment set hides — rays from the skin in a cone, blocked only by garment faces on the skin's own bones (kin steps and a fold glob widen "own"), the configuration's shapes applied first — and emit the Modular Avatar Delete carrier shape key at polygon granularity; `--whatif` reports (renders and an inspect blend on request), the real run writes the key into the base blend the linked body resolves to — a copy or in place — refusing a changed body. Behavior: `blender.md`. |
