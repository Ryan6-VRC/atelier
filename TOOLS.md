...
| `ReportShapeOverlap` | Same-mesh blendshape overlap over a co-active set the tool assembles — caller-passed ∪ worn-nonzero ∪ MA `ShapeChanger` reaction targets, the last being shapes at weight 0 at edit time the agent can't see. It reports each shape's touched-vertex footprint, pairwise containment (intersection over the smaller footprint), and a per-shape reaction/weight table; `MISSING` when an MA reaction type won't resolve, never a silent drop. Catches the double-subtraction a worn base `Shrink_*` and an outfit `ShapeChanger` stack over the same vertices — invisible to the render sheet and CheckSeam/CheckAvatar. A report, not a verdict: the agent rules wanted-vs-defect. |
