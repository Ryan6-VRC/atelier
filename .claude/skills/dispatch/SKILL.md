...
- `--delete-branch` never deletes the *local* branch. Delete it by hand only after `git diff <branch> main --diff-filter=A --name-only` comes back empty — diff, don't grep; spot-checking headings is how unmerged work gets mislabeled superseded.
