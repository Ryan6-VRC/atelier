...
Reclaim as a step in the loop — on merge, on early-stop, on abandonment — not a batch at the end; early worker termination is normal, not an error. On merge: squash + `--delete-branch`, ff local main, remove the worktree, cross the block off with a one-sentence note (git holds the detail), hand-register any tool-index change a worktree-skipping hook missed, and confirm sibling manifests moved in lockstep (`plugin.json` ↔ `marketplace.json` — the worker doc has workers bump both; a PR that bumped only one is the miss to catch).

The inbox file is the operator's call, not an autopilot step: surface its findings and let the operator pick what's worth a tracker entry, rather than transcribing the whole file into `docs/local/dispatch-plan.md`'s owed list — that transcription is itself the triage judgment §Draining a repair queue governs, and skipping it queues items nobody weighed (caught 2026-08-12: a queued finding the operator then had to explicitly veto). The one-sentence shipped-ledger line is still yours to add on sight, since it's a receipt, not a judgment call.
