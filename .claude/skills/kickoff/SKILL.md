...
Blocks live in `docs/local/kickoffs.md` (untracked). Shape: `## <ID> — <title>`, first body line `**Gate:** …` (launchable / blocked-on-X / operator-held — `grep -A1 '^## '` is the board), last line the terminal state (normally an open PR with the repo's gates green) and tier.
