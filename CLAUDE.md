...
- **`docs/osc.md`** — the avatar parameter wire: address families, wire types, the latching-vs-momentary contract, and the emulator's OSC surface against the client's. Read when a rig is driven or read over OSC, or when authoring anything an external app consumes. `vrc-bridge` keeps its own design record.
