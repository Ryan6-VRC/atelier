...
- **`docs/LAYOUT.md`** — AvatarProject folder conventions: untouched-`Vendor/` vs. our-work split, non-Unity files outside `Assets/`, and **§Vendor mutation** — the enumerated cases where writing under `Vendor/` is sanctioned. Read it before any deliberate write there, including a `.meta`.
...
9. **Never run Unity work in parallel** — one agent driving Unity at a time, one Unity call in flight, because every subagent shares the session's single MCP connection: parallel Unity work silently re-routes pins and serializes into timeouts. Parallel subagents are fine for everything else.
