...
5. **Verify intent, not just output.** Tests, scene checks, import checks, and diagnostics should prove the reason the behavior matters. A check that still passes after breaking the business or asset logic is wrong. **A check that does not guard a non-obvious breakage is deleted**: nothing for what fails obviously at first use, no assert on prose, no re-derivation of generated output, no test that certifies a helper's known bugs.
...
7. **Fail loud.** "Done" is wrong if anything was skipped silently. "Verified" is wrong if checks were skipped. Surface uncertainty, missing inputs, and named offenders. Loud means in the session's report, not in the repo: this rule licenses no new check, comment, or tag.
...
11. **If it works, it works.** A committed file never says who decided or when: no `(operator-ruled …)` tags, no numbered rulings, no spec or review-round citations, no dated retunes, no decision record. A design choice is what the code does plus a one-line why where the why is non-obvious. Provenance lives in git history and in `docs/local/` briefs, never in tracked prose.
