...
9. **Never run Unity work in parallel** — one agent driving Unity at a time, one Unity call in flight, **forks and subagents at any depth included, and read-only included**: every one of them shares the session's single MCP connection and the one mutable venue behind it, so parallel Unity work silently re-routes pins, serializes into timeouts, and interleaves edits neither side can see from inside. A read-only brief is not a safeguard — a fork sent to look can still author and can still enter play mode. Never spawn a Unity-touching subagent; parallel subagents are fine for everything else.
