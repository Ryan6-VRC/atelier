...
| `RenderAvatar` | Isolated Scene-View render of one avatar subtree, NDMF-preview-resolved (proxy-aware, so MA-reactive bodies render), framed from named world-axis angles to a contact-sheet PNG — operator-eye evidence for fit and clipping after a compose. Two doors over one capture core: `Capture` (the contact sheet), `CaptureDiff` (pinned-camera differential — changed-pixel count + cluster bbox, exact compare, no tolerance); `CaptureDiff` is `verify.md`'s sanctioned differential form. Grab in a call separate from any edit; OK carries `gate=armed` (rendered-current) or `gate=exempt` (nothing to certify), while an unsettled or backgrounded-editor preview fails the grab loud, so just re-grab. |
