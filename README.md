...
| [`weightpaint`](https://github.com/Ryan6-VRC/vrc-skills/blob/main/skills/weightpaint/SKILL.md) | Fix how a garment or hair follows the body: transfer the body's skin weights while every garment bone keeps its own, push a skin-poke region a millimetre off the skin, or fold a bone chain into its neighbours; measured by a bone sweep, ranked for your eye in an Editor A/B, and recorded as the recipe that made it. |
...
| `WriteDynamics` | Write a caller-authored VRC dynamics table onto a prefab or scene root: nodes (optionally with a physbone or collider), physbones created on an existing bone or removed, physbone root moves, physbone and collider field sets (curves included), constraint source tables and removals. The whole table is resolved first, and its one Undo group reverts whole on a refusal or a `whatIf` preview; in play it rewrites physbone and collider field sets and constraint weights live, and a play physbone field set is async: `WriteDynamics.Run`, then poll `WriteDynamics.Status()`. Contract: `unity-tools.md`. |
| `DrivePhysBones` | Pose bones in play mode, snapped or ramped, and sample every physbone chain under a root: tip travel from rest in root space and in each chain's own frame, jitter at mid and end of each hold, per-pose `ReportPenetration` counts and `RenderAvatar` frames. Async: `DrivePhysBones.Run`, then poll `DrivePhysBones.Status()`. Contract: `unity-tools.md`. |
