...
- **A blocking platform importer setting is ours to correct** — in place, on the vendor asset, by `ConformImportSettings` at the import door (`unity-tools.md`). Sanctioned on three counts: it **blocks** (an advisory never is), its offender status is **intrinsic to the asset** (the SDK's menu-icon rule fails that and stays the operator's), and it writes only the **`.meta`** — naming in its RunLog any change that reaches what ships — so it needs no sidecar, and a `.unitypackage` re-import or venue restore simply drops it.
