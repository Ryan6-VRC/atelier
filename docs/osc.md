...
Routes out, so nothing below is a second copy: parameter-name hazards and the compile advisory that surfaces them are `animator-schema.md`; the ~0.1 s sync tick, 8-bit quantization, and OSC's local-precision exception are `runtime.md` §Parameters; driving and reading OSC inside a play session is `emulator.md` §OSC.
...
- **Emission is change-only, per Unity frame, rate-limited in simulation time.** A static parameter goes quiet rather than streaming; `resendAllParameters` forces a full re-send. At a pinned `captureDeltaTime` of 11.9 ms the wire carried one burst per two frames (every frame at 32.5 ms), so a sub-20 ms replay reads per-frame series from its own truth log, not the wire.
- **One sender, one copy per change.** The socket bound to 9000 is what sends, so every emission arrives once, sourced from `127.0.0.1:9000`. The client instead opens two sender sockets and delivers every inbound message **twice** (`vrc-bridge/docs/design.md` §Inbound delivery semantics), so a listener that is not idempotent double-applies against the client and cannot be caught here — the one delivery property this venue does not reproduce (`verify.md`).
- **It emits more than its own config declares.** Parameters written through the SDK's animator-parameter-access path — contact-receiver and raycast outputs — reach the wire even when absent from the generated config, so an animator-only parameter is observable without appearing in any expression-parameters asset.
- **Inbound writes land on the same value the `.expressionValue` drive route writes** (`emulator.md` §Drive/observe). Two writers, one value; use one per session.
- **Addresses are built from the parameter name verbatim**, without the client's rewriting: the client replaces each space with `_` and serves only that address, 404ing the spaced spelling that is the only one resolving here. Cross-check anything name-sensitive against a real client.
