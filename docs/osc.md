...
- **`/avatar/change`** — **both directions**, carrying an avatar id as `,s`. Emitted when the worn avatar changes; sent *in*, it changes the worn avatar. The client accepts only ids in the player's favorites, recents, own uploads, or purchases, and an ineligible id does not swap. **The inbound direction carries no outcome, only an acknowledgement**: the client re-emits `/avatar/change` with the id it was *sent* within 5 ms, identically for an ineligible id and for a malformed one, and never emits again when the avatar actually loads (measured 2026-08-04, one client build). So the echo proves receipt and nothing more — a swap's success is not observable on this wire, and the worn avatar has to be read from the account's profile instead. A swap is accepted with the game menu open. **The reference pages document the outbound direction alone** (`docs/osc-avatar-parameters`, the community wiki), so read the inbound half off VRChat's patch notes: 2025.1.2 introduced it, 2025.4.2 widened it past favorites-only to match its own documentation. Do not "correct" this line from a reference page that omits it.
