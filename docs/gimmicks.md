...
goal, not to stack every pattern that fits.
...
- **DBT configuration states.** A state's motion is a Direct blend tree stacking single-purpose
  ...
  animator layers: Unity's per-layer cost is **super-linear**, so collapsing toggles into one DBT
  layer cuts frametime as well as clutter.
