...
The workspace reads its sub-repos freely — the tool-inventory sync, the venue provisioner and the prose fence all enumerate `vrc-*` — and that direction needs no ceremony, because knowing what it contains is what the container is for. The reverse direction is the governed one.

**A sub-repo reaches up into the workspace only for infrastructure it cannot own**: the shared test venue, its provisioner, the editor resolver, the disposable output pile. What those have in common is that none of them could ship as a package — a venue, a machine-path ladder and a scratch directory have no version to publish — and that is the boundary, not a list to extend by resemblance. Domain logic never travels this way: a helper two repos want that *could* ship as a VPM package must, in `vrc-unity-tools`, because that seam is versioned and this one is not.

The reach is explicit or it is not sanctioned — a root parameter that defaults by discovery and **refuses by name** when the workspace is absent, never a relative hop or a guessed fallback. `vrc-patterns/tools/gate.ps1` is the worked case, and its refusal is the whole contract test the seam gets: a checkout that cannot reach the workspace could not have reached the venue or the provisioner either, so a fallback would relocate the failure rather than survive it. No version negotiation exists across this seam by design — both sides are checked out together or neither runs.
