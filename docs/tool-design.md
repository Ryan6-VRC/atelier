...
- **One grammar across the family.** Same concept, same name — across tools, parameters, result keys, menu paths, docs. Same kind of result, same envelope — one diagnostic shape, not one per tool. A new tool's interface should be guessable from the last one; rename to converge rather than alias; name a heuristic as a heuristic, not a fact. On an `[AgentTool]` class this settles the door names: **the primary door is always `Run`** — the class already carries the verb (`<Verb><Subject>`, below), so the door never repeats it — and every other member is a secondary with its own subject, never a primary to guess at. The emitted verdict **label** names the action, not the door (`[RenderAvatar] Capture` / `CaptureDiff`), and does not follow it — a multi-door class would otherwise lose the only thing telling its console lines apart. `tools/sync_tool_inventory.py` asserts the `Run` half.
...
- **A sub-repo reaches up only for what cannot ship.** The workspace reads its sub-repos freely — knowing what it contains is what a container is for. The reach back takes only infrastructure with no version to publish (the test venue, its provisioner, the editor resolver, the scratch pile), through a root parameter that defaults by discovery and **refuses by name** when the workspace is absent — never a relative hop or a guessed fallback. Anything that *could* ship as a VPM package must, in `vrc-unity-tools`, where the seam is versioned; this one is not, and that refusal is the only contract test it gets, because both sides are checked out together or neither runs. `vrc-patterns/tools/gate.ps1` is the sole instance.
...
- **Deep**: pattern and reference entries — narrow precedent, usually narrow *because* it takes a working rig to express; routed from the doc that names it, reached via catalog.
- **Machine-fired**: checks, for traps whose biting circumstance is detectable (§Lifting).
- **Off the ladder**: venue READMEs, provenance sidecars, `docs/local/`. They hold what is true of one avatar or one campaign — its assets, names, rulings, per-item records. The test for a sentence there: would it still be true in an empty venue? If yes it is a mechanism of Unity, Blender, a framework or one of our tools, and its home is on the ladder, not here — a mechanism first met on a personal avatar is not a fact about that avatar. Domain depth the workspace declines to carry (cameras, shaders and particles, by the operator's standing call) stays off the ladder or goes to a pattern entry, never to a `docs/` file.

Every fact has one canonical home; every other mention links to it.

**A fact leaves the off-ladder tier by a pen and a drain, never a mid-item PR.** A writer who meets a mechanism mid-item writes it once, where it is needed, prefixed `LIFT→docs/<file>.md:` naming the target doc — no worktree, no PR, so nothing is parked. The marker is the fence for files `check_prose.py` cannot see: `grep -rn 'LIFT→' docs/local <venue>/Assets <venue>/Blender` must come back empty before a checkpoint is reported as closed. The drain runs at an item boundary as a dispatched session (`dispatched-work.md`): one PR per target doc, written under `write-for-agents`, and once merged each marker is replaced by a route to the landed section — after checking the venue's `git status`, since another session may hold it.
