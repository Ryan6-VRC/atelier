...
- Menu-relevant truth is **post-build only**: the descriptor's authored assets miss everything MA
  ...
  gate: `verify.md`).
- To read an **authored** menu chain cheaply, `AgentInspector.Snapshot(… followAssets:true)` and
  read the inlined `fields` — it follows the `subMenu` asset refs to the whole tree in one capture
  (great for vendor/raw asset-trios), no YAML grep. It reads the descriptor's authored assets only,
  so on a composed avatar it still misses MA/VRCFury-injected controls — the post-build live read
  above governs there. The dump is raw: this SDK line serializes control types as ~100-offset ints
  (102=Toggle, 103=SubMenu, 203=RadialPuppet), not the documented 0–5, and the tool does not decode
  them — identify controls by *shape* (subMenu ref vs `subParameters` vs flat value), not the literal.
