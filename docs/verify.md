...
does not. **One form is sanctioned**, report-the-matrix (`unity-tools.md`): **`CaptureDiff`** — an
exact pixel compare of a before/after pair around a camera pinned from frame A, reporting
`changed`/`bbox`/`identical=k/n`. It renders no verdict; the operator argues the call from the counts.
