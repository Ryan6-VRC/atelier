...
- **An `ArmatureLink` built by reflection or script is not the inspector's behaviour**, and two of its defaults put the prop somewhere you did not ask for (both measured; hand-authoring may be exempt — the inspector writes the same fields, but through its own UI paths). The model ships `linkTo` **already holding one entry, `HumanBodyBones.Hips`**, and the list is a first-resolvable-wins fallback chain, so an entry you append never runs and the prop links to Hips: clear the list before adding to it. Alignment then lives in `alignPosition`/`alignRotation`/`alignScale` — `keepBoneOffsets2` is `[Obsolete]`, read only by the model's version upgrade, so a component constructed fresh at the current version never consults it and setting it to `No` snaps nothing (measured: the anchor stayed 1.02 m off target). Independent of either field, seat the prop bone on the target bone's **rest pose** while authoring, so an offset the link does retain is identity.
