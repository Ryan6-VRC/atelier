...
**A page holds 8 controls and going over is a compile error, not an advisory.** The SDK's own menu inspector truncates `controls` to `VRCExpressionsMenu.MAX_CONTROLS` the moment a human opens the asset, so an over-long menu silently *loses* controls rather than degrading — split it into sub-menus. `MenuLimits.MaxControlsPerMenu` mirrors the SDK constant for the System.*-only validator; a real emit asserts the two agree and fails loud if the SDK ever moves it. The cap is **per page**, so a full page plus a sub-menu holding another full page is legal.
