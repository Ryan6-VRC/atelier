...
**The params asset.** The compiler emits one listing every declared param **except** VRC built-ins and `scratch:` ones — *unsynced included*, at zero sync-bit cost, so a human reading the merged FX sees them. A consumer references it by GUID (a VRCFury `FullController` names it in `prms:`), so **this list is the avatar's expression-parameter contract**: a VRC built-in costs nothing to omit because VRChat supplies it, while a dropped **custom** name is simply absent from it. Nothing to declare beyond `parameters:`; `vrc:` only tunes the entries.
...
- **Direct**: `directWeight: <paramName>` — a weight param left at its default `0` weights every child 0, and the document still compiles and lints clean; what the bindings then read is the rest-fill trap below.
...
**Trap — below a weight-sum of 1, a `direct` tree fills the shortfall from the binding's rest value, per binding.** The output is `Σ(wᵢ·vᵢ) + max(0, 1 − Σwᵢ)·rest`, where `wᵢ` counts only the children carrying *that* binding, so a weight-1 sibling on a different property closes nothing. At or above 1 there is no fill and no normalization — the sum is exact. For a parameter curve `rest` is the parameter's **declared default**, which is the whole reason the AAP idiom holds: an AAP declared `0` reads back `Σ(wᵢ·vᵢ)`, and any other default silently offsets every read. For a scene binding it is the value that binding carries with the state's motion not covering it — the scene's own on a lone layer; whether a layer stack fills from the stream below instead is not established. Write Defaults does not change the fill for a binding the state's own motion animates. A **discrete** binding (`GameObject.m_IsActive`, a renderer's `m_Enabled`) does not blend at all: it holds rest until the weight-sum reaches 1, so a toggle under a deficit never fires rather than firing weakly. Established for float transform channels, a material float, a parameter curve, and those two discrete bindings on a single layer; quaternion rotation and humanoid muscle curves are unmeasured.

**A weight-1 child on the same binding makes the sum exact — it does not restore the rest blend.** The output becomes `Σ(wᵢ·vᵢ) + ballast`, a constant term in place of the weight-proportional `(1 − Σwᵢ)·rest` it replaces, and nothing reproduces that blend; so choose the ballast as the constant the arithmetic wants. `0` where the leaves are absolute values summing to the answer, the seed value where the rig reads an offset from it (a proxy transform seeded off its parent, a constraint's resting offset) — zeroing one of those breaks the rig in the state that looked correct. Fold the write into an existing weight-1 child where one exists, since a new child adds its length to the duration formula above. None of this reaches a discrete binding: there a deficit means the write never lands, and the fix is to raise the weights rather than add a constant child. For scene bindings no compile-time check separates a harmful deficit from a harmless one — the discriminator is the rest value, which neither the compiler nor an out-of-project gate run has a scene to read. A parameter curve is the exception, its default being in the document.
...
The same `clips:` grammar lives in two places. **Inline** under a controller's own `clips:`, a clip embeds as a hidden sub-asset — the default. **In a clips file** — a document with a top-level `clips:` and **no `layers:`** — `CompileClips` (`animator.md`) emits each as a standalone, *visible* `.anim` a human can edit. A controller reaches one through the ordinary `motion: { ref: "<outDir>/<clip>.anim" }` (§motions), no new directive, so one controller mixes embedded and external clips freely.
...
Two consequences before relying on the field. **The gate cannot see an icon**, so an entry whose `built/` and yaml disagree about one still passes; and an out-of-project compile cannot check that an icon is even an image, which also waits for the in-project compile. Icons survive composition onto a host avatar (`menus.md`).
...
- **`timeParameterActive` with an empty parameter** → normalized to unbound motion time.
