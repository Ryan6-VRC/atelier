...
**Refused — a driver may not touch a parameter a clip binds, in either direction.** Not the timing race the old wording described: the animation system *owns* a bound parameter (`runtime.md` has the measured scope and its venue). A driver write (`set`/`add`/`copy`/`random`) to such a parameter reaches no animator reader, and a `copy` whose **source** is one reads the parameter's declared default rather than the value the clip is producing — the direction the compiler used to miss entirely. `RuleNonFloatParamCurve`'s sibling `driverOnAnimatedParam` fails the compile on both. Drive a parameter no clip animates, or move the value with a blend tree; if a state change is what you need, a transition condition *can* read an AAP, as can a blend tree. **One legitimate case the refusal still catches:** a driver write whose consumer is *outside* the animator — an OSC-out signal or a `saved:` value, which the expression-parameter copy is inferred to still carry. The construct is real; give that driver a parameter no clip in this controller binds.
