...
**Trap — below a weight-sum of 1, a `direct` tree blends the shortfall into the binding's rest value, per binding.** The output is `Σ(wᵢ·vᵢ) + max(0, 1 − Σwᵢ)·rest`, where `wᵢ` counts only the children carrying *that* binding, so a weight-1 sibling on a different property closes nothing. At or above 1 there is no fill and no normalization — the sum is exact, which is what makes a ballast child free. `rest` is the scene's value for a scene binding and the parameter's **declared default** for a parameter curve: the AAP idiom holds only because an AAP is declared `0`, and any other default silently offsets every read. Write Defaults does not enter into it — for a binding the state's own motion animates, the fill is identical at either setting. A **discrete** binding (`GameObject.m_IsActive`, a `Behaviour`'s `m_Enabled`) does not blend at all: it holds rest until the weight-sum reaches 1, so a toggle under a deficit never fires rather than firing weakly.

**Close a deficit with a child on the same binding at a statically-constant weight of 1**, writing the value the shortfall should land on — `0` where the arithmetic wants a clean sum, the rest value itself where the rig depends on it. A blind `0` is its own defect: where the rest is load-bearing (a proxy transform seeded off its parent, a constraint's resting offset), zeroing it breaks the rig in the state that looked correct. Fold the write into an existing weight-1 child where one exists, since a new child adds its length to the duration formula above. No compile-time check separates a harmful deficit from a harmless one — the discriminator is the binding's rest value, and neither the compiler nor an out-of-project gate run has a scene to read it from.
