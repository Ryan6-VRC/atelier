...
- **AAO `MergeBone` inside the chain.** Merges the bone upward into its own parent: destroys the transform, reparents its children world-preserving, remaps skin weights and ignore lists (`MergeBoneProcessor.MapIgnoreTransforms`). One transform per bone, so the usual shape is every second bone of a chain; felt, since the chain is shorter, and a merged bone with children changes which bone carries their motion. Outside every chain it moves `boneCount` only. The type is internal — add by reflection, and set `avoidNameConflict` deliberately, since true renames every reparented child.
